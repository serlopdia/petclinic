...
class PetServiceTests {        
	...
	public void shouldInsertPetIntoDatabaseAndGenerateId() {
		...
		owner1.addPet(pet);
		assertThat(owner1.getPets().size()).isEqualTo(found + 1);
	...
	public void shouldThrowExceptionInsertingPetsWithTheSameName() {
		...
		Assertions.assertThrows(DuplicatedPetNameException.class, () ->{
			owner1.addPet(anotherPetWithTheSameName);
	...
	void shouldFindVisitsByPetId() throws Exception {
		Collection<Visit> visits = this.petService.findVisitsByPetId(4);
		assertThat(visits.size()).isEqualTo(1);
		Visit[] visitArr = visits.toArray(new Visit[visits.size()]);
		assertThat(visitArr[0].getPet()).isNotNull();
		assertThat(visitArr[0].getDate()).isNotNull();
		assertThat(visitArr[0].getPet().getId()).isEqualTo(4);
	}
